...
.getProperty<-function(x,property){
  ...
  if(is.data.frame(x)){
    res=lapply(seq_len(ncol(x)),function(i)func(x[,i]))
  }else{
    res=func(x)
    #res=list(res)
  }
...
getSharedProperty<-function(x,as.list=FALSE){
  if(is.data.frame(x)){
    res=lapply(seq_len(ncol(x)),function(i)getSharedProperty(x[,i]))
  }else{
    dataid=.dataId(x)
    res=structure(C_getDataInfo(dataid),class="sharedProperty",names=dataInfoName)
    if(as.list){
      res=list(res)
    }
  }
