...
Currently, the package supports `raw`, `logical`, `integer` and `double` data types. `character` is not supported. The data structure `atomic`(aka `vector`), `matrix`, `data.frame` and `list` can be shared by the function `share`. The function `share` is an S4 generic, developers can define an S4 `share` function to support their data structures.

Please note that sharing a list object will not sharing the list itself, but instead sharing each element of the list object. Therefore, adding or replace an element in a list will not implicitly share the element. Users should be aware of such operations. Since a data frame is fundamentally a list object, sharing a data frame will follow the same principle as sharing a list. 
...
is.shared(A_shr)
```
For an atomic object, `is.shared` returns a logical value indicating whether the object is a shared object. For a list object, it returns a list of logical values with each element represent the corresponding element in the list object.

There are several properties with the shared object, one can check them via
```{r}
## get a summary report
getSharedObjectProperty(A_shr)
